...
fid_im = fopen('File1_fd20_1_ofdm_only.pcm');
Num_of_samples_q = 132000;
data = fread(fid_im,Num_of_samples_q,'int16');
fclose(fid_im);
data_i = downsample(data,2);
data_q = downsample(data,2,1);
data_samples = complex(data_i,data_q);

Ns = Num_of_samples_q/2;
n = 1 : Ns;
fs = Ns;
dt = 1/fs;
t = n*dt;
...
grid on
hold on
plot(abs(fftshift(fft(data_samples(1:512)))))
...
data_samples_fpga = complex(data_re(1138:1649),data_im(1138:1649));
